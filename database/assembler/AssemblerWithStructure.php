...
use core\database\result\Result, 
	...
	core\database\assembler\structure\EntityStructure;

class AssemblerWithStructure extends Assembler {

	public $structure;
	public $groupingSpecs = [];
	public $interceptors = [];
	
	public function __construct($structure, $groupingSpecs = null, $interceptors = null) {
		$this->structure = ($structure instanceof EntityStructure) ? 
			$structure : new EntityStructure($structure);
		
		if ($groupingSpecs) {
			if (!is_array($groupingSpecs)) 
				$groupingSpecs = ['main*' => $groupingSpecs];
			
			$this->setGroupingSpecs($groupingSpecs);
		}
	...
	public function setGroupingSpecs(array $groupingSpecs) {
		foreach ($groupingSpecs as $key => $groupingSpec) {
			$isCollection = false;
			...
			$this->groupingSpecs[$key] = [$groupingSpec, $isCollection];					
		}
	
		return $this;
	}
	
	public function getGroupingSpec($key) {
		return isset($this->groupingSpecs[$key]) ? $this->groupingSpecs[$key] : null;
	}
	...
	public function getInterceptor($key) {
		return isset($this->interceptors[$key]) ? $this->interceptors[$key] : null;
	}
	...
	public function assemble(Result $result) {
		...
		return new Result(new AssociativeArraySource(
		 	$this->removeIdentifiers($entities, $this->getGroupingSpec('main'), $this->getInterceptor('main'))
		));
	}
	...
	*/
	...
	protected function removeIdentifiers($entities, $groupingSpec, $interceptor) {
		$result = [];
		foreach ($entities as $entity) {
			foreach ($entity as $key => $value) {
				if (is_array($value) && $key != 'junction') {
					...
					$entity[$location] = $this->removeIdentifiers(
						$value, $this->getGroupingSpec($key), $this->getInterceptor($key));
			...
			if ($groupingSpec) {	
				$key = is_string($groupingSpec[0]) ? $entity[$groupingSpec[0]] : $groupingSpec[0]($entity);
